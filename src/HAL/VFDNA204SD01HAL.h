...
// VFDNA204SD01HAL: HAL for Noritake NA204SD01 (20x4) VFD module
// Uses SD01-style module control codes: Reset, Clear, Home, Position (0x10 + addr),
// Dimming (0x04 + code), and cursor modes.
class VFDNA204SD01HAL : public IVFDHAL {
public:
    VFDNA204SD01HAL();
    ~VFDNA204SD01HAL() override = default;
...
private:
    ...
    bool _posLinear(uint8_t addr);     // 0x10 + addr (0x00..0x4F for 20x4)
    bool _posRowCol(uint8_t row, uint8_t col);
    bool _cmdBackSpace();              // 0x08
    bool _cmdHtab();                   // 0x09
    bool _cmdCR();                     // 0x0D (acts as clear on this family)
